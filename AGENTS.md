# AGENTS.md — Synchronous Generator Simulator (LEVEL 1)

File ini memberi panduan kepada agen coding (Codex / Codebuff / Freebuff CLI / dll.)
...
```
User request → Baca AGENTS.md → Cek relevant tests → Implementasi → Run tests → Verify pass → Commit
```
...
9. **Push sebelum ganti sesi (WAJIB).** Jangan menunda push dengan alasan "menunggu
   ...
   agar cabang kanonik memuat fix fisika yang AGENTS.md sitasi.
